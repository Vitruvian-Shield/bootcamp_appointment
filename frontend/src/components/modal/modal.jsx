...
export default function CitySelectModal({ open, handleOpen, handleClose }) {
  ...
  useEffect(() => {
    const fetchCities = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/medicine/locations/', {
          params: { search },
        });
    ...
    <Box>
      ...
      >
        <Box sx={style}>
          ...
          >
            با انتخاب شهر، اسامی متخصصان مربوطه را مشاهده نمایید.
          </Typography>
          <TextField
            value={search}
